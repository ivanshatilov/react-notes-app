...
const Note: React.FC<NoteProps> = ({value, id, date, confirmed, editable}) => {
    ...
    return (
        <div className={`border-2 rounded-md border-gray-300 pl-5 m-2 h-48 lg:h-24 overflow-clip flex gap-0 ${confirmed && "bg-green-100 border-green-300"} ${editable && "bg-yellow-100 border-yellow-200"}`}>
            <textarea id='note' name="textarea" disabled={!editable} value={value} onChange={updateNoteHandler} className={`bg-transparent grow block text-md text-gray-700 pr-10 tracking-tight font-semibold resize-none overflow-auto whitespace-pre-line outline-none h-full mt-3 disabled:bg-transparent ${confirmed && "text-green-700"} ${editable && "text-yellow-800"}`} maxLength={128}/>
                <div className={`flex flex-col h-full bg-gray-100 items-center px-4 py-2 ${confirmed && "bg-green-200"} ${editable && "bg-yellow-200"}`}>
                    <div className='flex flex-col mt-2 lg:mt-0 lg:flex-row gap-3 h-full bg-transparent items-center px-4'>
